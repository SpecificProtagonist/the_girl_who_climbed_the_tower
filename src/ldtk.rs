...
/// users).
#[derive(Debug, Serialize, Deserialize, Asset, TypePath, Resource)]
...
pub struct LayerInstance {
    ...
    /// start at 1.  
    /// The array size is `c_width` x `c_heigth` cells.
    ...
    /// the `LayerDef` optional offset, so you should probably prefer using `px_total_offset_x`
    /// which contains the total offset value)
    pub px_offset_x: i32,
    /// Y offset in pixels to render this layer, usually 0 (IMPORTANT: this should be added to
    /// the `LayerDef` optional offset, so you should probably prefer using `px_total_offset_y`
    /// which contains the total offset value)
    pub px_offset_y: i32,
...
pub struct NeighbourLevel {
    ...
    /// `e`ast).  
    /// Since 1.4.0, this value can also be `<` (neighbour depth is lower), `>`
